...
- **Security exceptions pass through.** The clean `UNAUTHORIZED` and `FORBIDDEN` errors from class 4 come from the dedicated resolver Spring for GraphQL ships for security exceptions — but handler methods in this advice run *first*, so a catch-all for `Exception` would consume `AccessDeniedException` before that resolver ever sees it, and every denied request would come back as a meaningless `INTERNAL_ERROR`. The guard at the top of `handleUnhandled` prevents that: a handler that returns `null` leaves the exception unresolved, and resolution moves on to the next resolver in the chain. Declare what you handle; yield what you do not.

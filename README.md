...
> [!IMPORTANT]
> **This is the workshop starting point, not a finished project.** It contains the Spring Boot scaffolding and a React frontend, but no GraphQL code at all: we build the API together during the workshop. This is deliberate. `main` is the repository's default branch so that cloning it puts you exactly where the workshop begins.
>
> The complete, finished application lives on the **`completed`** branch. Each class also has a **`checkpoint_N`** branch with the code as it stands at the end of that class, so you can catch up or compare at any point. The step-by-step instructions are in the [`workshop/`](workshop/) folder.
